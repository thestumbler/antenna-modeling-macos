// Global variables
int ihgt; // index, height
int ilen; // index, len
int iwid; // index, width
int idia; // index, diameter
int ispc; // index, spacing
...
void get_many( real *many, real a, real b, real d ) {
  many[0]=a;
  many[1]=b;
  many[2]=c;
}
...
// y-axis is broadside.
void ant_g5rv_length_set( real *ant, real len ) { ant[ilen]=len; }
...
element make_g5rv( transform t ) {
	...
	return wirev( t, vect( -feed_spacing, 0, height-feed_length ), 
                     vect(  feed_spacing, 0, height-feed_length ), 
                 feed_wire, ndsegs ) ;
}

element make_dipole ( transform t, real h, real l ) {

	// height = 40' ;
	// length = 5.0 ;
	return wire( 0, -l, h, 0, l, h, #14, 21 ) ;
}
